...
class ImportPqrsfCsv extends Command
{
    ...
    private const CSV_ID_KEY = 'csv_id';

    private const VALID_STATUSES = ['pending', 'validated', 'sent'];
    ...
    public function handle(): int
    {
        ...
        if (! in_array($status, self::VALID_STATUSES, true)) {
            $this->error('El estado debe ser uno de: '.implode(', ', self::VALID_STATUSES));
        ...
        if ($errors !== []) {
            ...
            if (count($errors) > 20) {
                $this->line('Errores restantes: '.(count($errors) - 20));
            }
        ...
        $this->line('Registros leidos: '.count($records));
        $this->line('Registros nuevos: '.count($recordsToImport));
        $this->line('Duplicados omitidos: '.(count($records) - count($recordsToImport)));
        $this->line('Sedes nuevas: '.count($missingSedes));

        if ($missingSedes !== []) {
            $this->line('Sedes que se crearan: '.implode(', ', $missingSedes));
        }
        ...
        $this->info("Importacion completada. Registros importados: {$imported}. Sedes creadas: {$createdSedes}.");
